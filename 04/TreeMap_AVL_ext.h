...
#ifndef TREEMAP_AVL_EXT_H_
#define TREEMAP_AVL_EXT_H_
...
protected:
    ...
    struct TreeNode {
        ...
        TreeNode(clave_valor const& e, Link i = nullptr, Link d = nullptr,
                 int alt = 1, size_t tam = 1) : cv(e), iz(i), dr(d), altura(alt), tam_i(tam) {}
    };
...
public:
	...
	bool encontrar(int pos, Clave & cl) const{
		return encontrarClave(pos, raiz, cl);
	}
...
protected:
    ...
    static void subirMenor(Link & a, Link b, Link padre) {
        if (b->iz != nullptr){
			...
			a->tam_i = a->izq->tam_i + 1;
        }
    }

	bool encontrarClave(size_t pos, Link a, Clave & sol) const{
		if (pos > nelems)
			return false;
		else{
			if (a->tam_i > pos)
				return encontrarClave(pos, a->iz, sol);
			else if (a->tam_i < pos)
				return encontrarClave(pos-a->tam_i, a->dr, sol);
			else {
				sol = a->cv.clave; 
				return true;
			}
